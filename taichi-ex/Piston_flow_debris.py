...
block_position = ti.Vector([5.0, 3.0])  # Position of the block's center
block_size = ti.Vector([4.0, 2.0])  # Size of the block (width, height)
...
def init_particles():
    for i in range(num_particles):
        # Initialize particle positions within the simulation domain
        delta = h_ * 0.8
        ...
        # Initialize particle velocities with some randomness
        for c in ti.static(range(dim)):
            velocities[i][c] = (ti.random() - 0.5) * 4
        
          
        # Check if the particle lies within the block's region
        if (positions[i] - block_position).norm() < (block_size * 0.5).norm():
            # If the particle is within the block, set its velocity to zero
            velocities[i] = ti.Vector([0.0, 0.0])
            # You might want to adjust other properties like mass, etc.
