...
DIMENSIONS = 3 # DIMENSIONS, 2D or 3D
...
n_grid_total = int(ti.max(n_grid_x,1) * ti.max(n_grid_y,1) * ti.max(n_grid_z,1))
...
piston_pos = np.array([0.0, 0.0, 0.0], dtype=float)
...
board_states = ti.Vector.field(DIMENSIONS, dtype=float, shape=())
time = 0.0

#Define some parameters we would like to track
data_to_save = [] # Used for saving positional data for particles 
v_data_to_save = [] # Used for saving velocity data for particles 

#bounds = [[0.0, 0.9], [0.0, 0.5]]
bounds = [[0.0, 0.9], [0.0, 0.5], [0.1, 0.9]] # For 3D
vel_mean = []
vel_std = []
acc_mean = []
acc_std = []
...
grid_tuple = (n_grid_x, n_grid_y, n_grid_z) #if DIMENSIONS == 2 else  (n_grid, n_grid, n_grid)
...
def substep():
    ...
    for p in x:  # grid to particle (G2P)
        ...
        for i, j, k in ti.static(ti.ndrange(3, 3)):
...
def move_board_solitary():
    ...
    b[3] += dt  # Assuming b[3] is used as a time counter
    
    # Reset the time counter if it exceeds twice the piston period
    if b[3] >= 2 * piston_period:
        b[3] = 0

    # Update the piston position using the error function approximation
    soliton_wave = piston_amplitude * ((erf_approx(t - 2.5 - 1e-2) + 1.0) / 2.0)
    
    # Update x-position (assuming piston moves along x-axis)
    b[0] += soliton_wave
    
    # Ensure the piston stays within the boundaries
    b[0] = ti.max(piston_pos[0], ti.min(b[0], piston_pos[0] + piston_amplitude))
    
    # Y and Z positions remain constant (assuming piston only moves in X direction)
    b[1] = piston_pos[1]
    b[2] = piston_pos[2]
...
def reset():
    ...
    board_states[None] = ti.Vector([float(piston_pos[0]), float(piston_pos[1]), float(piston_pos[2])])
...
window = ti.ui.Window("3D MPM Simulation", (1024, 1024), vsync=True)
...
while window.running:
    ...
    # Draw the particles
    scene.particles(x, radius=0.005, color=(0.5, 0.42, 0.8))

    # Draw the ground plane
    scene.mesh(ti.static(plane_verts), ti.static(plane_indices), color=(0.5, 0.5, 0.4))
...
'''
plane_verts = ti.Vector.field(3, dtype=ti.f32, shape=4)
plane_indices = ti.field(int, shape=6)

@ti.kernel
def init_plane():
    plane_verts[0] = vec3(0, 0, 0)
    plane_verts[1] = vec3(1, 0, 0)
    plane_verts[2] = vec3(1, 0, 1)
    plane_verts[3] = vec3(0, 0, 1)
    plane_indices[0] = 0
    plane_indices[1] = 1
    plane_indices[2] = 2
    plane_indices[3] = 2
